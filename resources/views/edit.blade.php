...
@section('content')
    ...
    <header class="header">
        <div class="header__inner container">
            ...
            <form class="todo-form" method="POST" action="{{ route('todo.update.post', [$todo->id]) }}">
                @csrf
                <div class="todo-form__group">
                    <input type="text" name="title" value="{{ $todo->title }}" required class="todo-form__input"
                        placeholder="Todo Title..." />
                </div>
                <div class="todo-form__group">
                    <textarea name="short_desc" class="todo-form__input todo-form__text_area " id=""
                        placeholder="Short Description" cols="30" rows="10">
                        {{ $todo->short_desc }}
                    </textarea>
